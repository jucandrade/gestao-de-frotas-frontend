...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(data ?? { message: "Erro ao listar perfis" }, { status: response.status });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(data ?? { message: "Erro ao criar perfil" }, { status: response.status });
    }
