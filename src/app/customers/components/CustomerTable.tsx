...
export default function CustomerTable() {
  ...
  const filtered = customers.filter((customer) => {
    ...
    return (
      ...
      (cnpjTerm.length > 0 && customer.cnpj?.includes(cnpjTerm))
    );
  });
              ...
              {filtered.map((customer) => (
                ...
                >
                  <td className="px-4 py-3">
                    ...
                    {customer.tradeName ? (
                      <div className="text-xs text-gray-500">{customer.tradeName}</div>
                    ) : null}
                  </td>
                  <td className="px-4 py-3 text-gray-600">{customer.cnpj ? formatCnpj(customer.cnpj) : "—"}</td>
                  <td className="hidden px-4 py-3 text-gray-600 md:table-cell">{customer.city || "—"}</td>
                  <td className="hidden px-4 py-3 text-gray-600 lg:table-cell">
                    {customer.mainContact || customer.phone || "—"}
                  </td>
                  <td className="px-4 py-3">
